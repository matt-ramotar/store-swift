...
final class StreamHandlerTests: XCTestCase {
    ...
    private var testDatabase = Dictionary<String, TestLocalDataModel>()
    
    private func createSourceOfTruth() -> AnySourceOfTruth<String, TestLocalDataModel, TestDataModel> {
        return SourceOfTruthFactory<String, TestLocalDataModel, TestDataModel>.make(read: { key in
            guard let localData = self.testDatabase[key] else {
                let outputData: TestDataModel? = nil
                return Just(outputData).setFailureType(to: StoreError.self).eraseToAnyPublisher()
            }
            
            let outputData = TestDataModel(id: localData.id, name: localData.name, title: localData.title, ttl: localData.ttl)
            return Just(outputData).setFailureType(to: StoreError.self).eraseToAnyPublisher()
        }, write: {key, localData in
            self.testDatabase[key] = localData
        })
    }

    private func setUpStreamHandler(
        memoryCache: AnyCache<String, TestDataModel> = CacheBuilder<String, TestDataModel>().build(),
        sourceOfTruth: AnySourceOfTruth<String, TestLocalDataModel, TestDataModel>? = nil
    ) {
        ...
        let sourceOfTruth = sourceOfTruth ?? createSourceOfTruth()
        
        let validator = ValidatorFactory<TestDataModel>.make({ output in output.ttl > Date.now})
        
        streamHandler = StreamHandlerImpl(fetcher: fetcher, converter: converter, sourceOfTruth: sourceOfTruth, validator: validator, memoryCache: memoryCache)
    }
    ...
    func testStreamFromCachedRequestWithMemoryCacheButDataOnlyInSourceOfTruth() async throws {
        setUpStreamHandler()
        ttl = Date.distantFuture
        ...
        let expectedNumberOfResponses = 3
        let expectation = expectation(description: "Correct number of responses")

        let expectedData = TestDataModel(id: id, name: id, title: id, ttl: self.ttl)
        let localData = TestLocalDataModel(id: id, name: id, title: id, ttl: self.ttl)
        
        let sourceOfTruth = createSourceOfTruth()
        try await sourceOfTruth.write(key: id, value: localData)
        ...
        XCTAssertEqual(receivedEvents.get(0), StoreReadResponse.loading(origin: .cache))
        XCTAssertEqual(receivedEvents.get(1), StoreReadResponse.loading(origin: .sourceOfTruth))
        XCTAssertEqual(receivedEvents.get(2), StoreReadResponse.data(value: expectedData, origin: .sourceOfTruth ))
    }
